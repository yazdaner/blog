                  ...
                  <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-muted">
                            ...
                            <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#show-{{$comment->id}}">
                                نمایش
                            </button>
                        </td>
                        <td>
                            <a class="btn btn-sm {{$comment->getRawOriginal('is_approved') ? 'btn-success' : 'btn-danger'}}" onclick="updateItem(event,{{$comment->id}})">{{$comment->is_approved}}</a>
                            <form action="{{route('admin.comments.update',$comment->id)}}" method="post" id="update-form-{{$comment->id}}">
                                @csrf
                                @method('put')
                            </form>
                        </td>
                        <td>
                           <div class="dropdown">
                                ...
                                <ul class="dropdown-menu">
                                    <li>
                                        <a class="dropdown-item" href="" onclick="destroyItem(event,{{$comment->id}})">حذف</a>
                                        <form class="d-inline" action="{{route('admin.comments.destroy',$comment)}}" method="POST" id="delete-form-{{$comment->id}}">
                                            @csrf
                                            @method('delete')
                                        </form>
                                    </li>
