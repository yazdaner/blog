<div x-show="open{{$reply->parent->id ?? $comment->id}}" x-data="{ open{{$reply->id}}: false }">
    ...
    <div class="comment d-flex align-items-start m-5">
        ...
        <div class="w-100">
            ...
            <p class="lh-lg">{{$reply->content}}</p>
            <div class="d-flex align-items-start justify-content-between">
                <a class="btn btn-primary" onclick="setReplyValue({{$reply->id}},'{{$reply->user->name}}')" href="#response">پاسخ</a>
                @if ($reply->approvedReplies->count() > 0)
                    <button class="btn btn-outline-secondary" x-on:click="open{{$reply->id}} = ! open{{$reply->id}}">نمایش پاسخ ها</button>
                @endif
            </div>
        </div>
